...
import { db, addDoc, setDoc, doc, collection } from "../../firebase";
...
const SGPAForm = () => {
  ...
  const onSubmit = async (values, { resetForm }) => {
    const calculatedSGPA = calculatedSGPA(values.subjects);
    ...
    try {
      if (editMode && currentRecordId) {
        await setDoc(doc(db, "gpaRecord", currentRecordId), recordData);
      } else {
        await addDoc(collection(db, "gpaRecord"), recordData);
      }
    } catch (error) {
  ...
  return (
    <div className="sgpa-container">
      ...
      >
        {({ values }) => (
          <Form className="sgpa-form">
            <div className="sgpa-form-group">
              <label htmlFor="student-name">Student Name:</label>
              <Field type="text" name="student-name" />
              <ErrorMessage name="student-name" component="div" />
            </div>
            <div className="sgpa-form-group">
      ...
      {sgpa && (
        <div>
          <h3>SGPA: {sgpa}</h3>
          <button onClick={() => handEditRecord(currentRecordId)}>Edit</button>
        </div>
      )}
    </div>
  );
